...
public class ShepEntity extends BaseTime {
    ...
    @Builder.Default
    private int shep_hunger = 1;                // 양배고픔 -1 : 배고픔 , 0 : 보통 , 1 : 배부름
    @Column( nullable = false )
    @Builder.Default
    private int shep_fur = 1;                   // 양털상태 -1 : 털 많음 , 0 : 털 보통 , 1 : 털 없음
    @Column( nullable = false )
    @Builder.Default
    private int shep_exist = 0;                 // 양존재여부 1 : 울타리에 있음 , 0 : 목장에 있음 , -1 : 늑대에 쫓기는 중(사용자 소유가 아님)
